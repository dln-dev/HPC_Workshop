...
int main(void) {
	...
	CMatrix C = A;
	CMatrix R = CMatrix(20000, 1000, 1, 100);
	CMatrix S = CMatrix(1000, 20000, 1, 100);
	...
	tpoint t2 = timestamp();
	long int time = duration<chrono::milliseconds>(t1, t2);

	cout << "computing time ~ " << time/1000 << "," << time%1000 << " s." << endl;
